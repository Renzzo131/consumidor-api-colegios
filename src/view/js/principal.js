...
function generar_texto_paginacion(total, cantidad_mostrar) {
    let actual = parseInt(document.getElementById('pagina').value);
    let paginas = Math.ceil(total / cantidad_mostrar);
    let iniciar = (actual - 1) * cantidad_mostrar;
    let fin = iniciar + parseInt(cantidad_mostrar);

    if (fin > total) fin = total; // Evita pasarte del total

    let texto = `<label>Mostrando del ${iniciar + 1} al ${fin} de un total de ${total} registros</label>`;
    return texto;
}
